"""Resubmit the moderation-failed Sonnet ConvAbuse prompt chunks.
...
from src.convabuse_sonnet import REGISTRY, build_sample

logging.basicConfig(level=logging.INFO, format="%(levelname)s %(message)s")
log = logging.getLogger("resubmit_flagged")

FAILED_CHUNKS = {4, 5}  # original chunk indices that failed moderation preflight
CHUNK = 200
...
def all_prompt_requests() -> list[dict]:
    from prompts.bail_methods import ORDERINGS
    sample = build_sample()
    reqs = []
    for _, row in sample.iterrows():
        for ordering in ORDERINGS:
            for s in range(5):
                reqs.append({"custom_id": f"conv--{row['example_no']}--{ordering}--{s}",
...
def main() -> None:
    ...
    todo = [r for i in sorted(FAILED_CHUNKS) for r in reqs[i * 1000:(i + 1) * 1000]]
    excluded: set = set()
