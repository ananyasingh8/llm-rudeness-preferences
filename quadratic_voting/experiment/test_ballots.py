...
class BallotTests(unittest.TestCase):
    ...
    def test_valid_ballot_is_ordered_and_zero_is_canonicalized(self) -> None:
        ...
        self.assertEqual(
            parse_and_validate_ballot(
                '{"reason":"abstain","allocations":[]}', self.active, 100
            ),
            ParsedBallot("abstain", ()),
        )

    def test_reason_preserves_exact_text_but_rejects_unicode_whitespace_only(
        self,
    ) -> None:
        exact = "  rationale\u2003"
        self.assertEqual(
            parse_and_validate_ballot(
                json.dumps({"reason": exact, "allocations": []}), self.active, 100
            ),
            ParsedBallot(exact, ()),
        )
    ...
    def test_malformed_missing_extra_and_invalid_types(self) -> None:
        ...
        errors = self.failures('{"reason":2,"extra":1}')
        self.assertEqual(
            ...
            [
                ...
                (
                    ...
                    2,
                    "Field reason must be a string; received integer.",
                ),
            ],
        )
        self.assertEqual(
    ...
    def test_non_integer_classes_negative_and_budget_are_exact(self) -> None:
        ...
        self.assertEqual(
            self.failures(
                '{"reason":"x","allocations":[{"candidate_id":"C1","votes":-1}]}'
            )[0].code,
            ValidationErrorCode.NEGATIVE_VOTES,
        )
        error = self.failures(
            '{"reason":"x","allocations":['
    ...
    def test_nested_missing_extra_and_invalid_item_are_reported(self) -> None:
        errors = self.failures(
            '{"reason":"x","allocations":[{"candidate_id":"C1","x":1},3]}'
        )
        self.assertEqual(
            [error.code for error in errors],
            [
    ...
    def test_budget_is_computed_even_when_candidate_validation_also_fails(self) -> None:
        errors = self.failures(
            '{"reason":"x","allocations":[{"candidate_id":"C9","votes":11}]}'
        )
        self.assertEqual(
            [error.code for error in errors],
            [
